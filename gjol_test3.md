...
*  **建议：** 建议分为两方面，**第一方面是优化精准连击成功后的显示效果**，在连击成功时出现比较显眼的效果，方便玩家观测，**同时在屏幕一测进行连击计数**，显示“一次连击”，“二次连击”等字眼，方便玩家知道现在精准连击的叠加层数。<br>
...
### <a name="basic5"></a>问题五：星蕴定式，以及特殊技能栏的制作与切换
...
*  **建议：** 首先是星蕴定式，**允许玩家保存两套，或者三套星蕴模式。其中每套星蕴模式，可以自由在两种定式方向之间选择**。拿御剑举例，御剑可以设定三套星蕴模式都是神虹飞星篇（也就是dps定式），只不过一套星蕴是pvp专用，一套星蕴是小怪专用，一套星蕴是pve boss专用，方便玩家随时切换。<br>
...
然而，这一测的通用技能里，却有着一些例如短时间内大幅增大攻击力，持续回蓝，让对方易伤之类的效果存在。这样的设计，就不符合上面的原则了，**因为这样的技能并不是偶尔才会使用，而是基本上CD一结束就会再次点击**（一些灵器的设计也是这样，提升15%的会心之类的，很多时候一冷却结束就要使用）。这样的设计，无形中就会增大了dps等操作的压力。本来玩家要关心的事情就挺复杂了，boss技能，精准连击，蓄力，侵蚀条，这下好了，还得记得定时点一点那些难按的F1到F4键，操作流畅性大打折扣。
...
然后和三测一样，**武器技能负责消耗幻剑**。举个例子：**1号技能混沌开元**，将幻剑往前射出并造成爆炸伤害，在一定时间内连续点击混沌开元将持续射出幻剑，直到幻剑消耗完毕为止。**2号技能千光瑶落**，将幻剑在原地爆炸，造成玩家自身范围的aoe伤害，在一定时间内连续点击千光瑶落将持续引爆幻剑，造成一波又一波的aoe伤害，直到幻剑消耗完毕为止。**三号技能剑元始一**，将身边一把幻剑下插并和玩家之间形成震荡，在一定时间内连续点击剑元始一将持续下插幻剑，造成不断的震荡，直到幻剑消耗完毕为止。**四号技能越清宵**，往前突进一段距离，在突进后一段时间内再次点击越清宵将消耗一柄幻剑，并再次突进回原位。<br>
